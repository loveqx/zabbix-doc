...
run_ch_query()
{
	...
	/usr/bin/clickhouse-client -h "$CH_HOST" -u "ckuser" --password "$CH_PASSWORD" -d "$DATABASE" -q "$SQL"
}

##
## Fetch metric by name from ClickHouse
##
run_ch_metric_command()
{
